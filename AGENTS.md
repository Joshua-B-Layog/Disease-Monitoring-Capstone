...
## PWA & Offline Support (Phase 1 + Phase 2)
...
- IndexedDB via `dexie.js` stores: `cases`, `diseases`, `barangays`, `referenceData`, `syncQueue`
- `offlineSync.js` — cache/fallback helpers; `db.js` — Dexie schema
- Dashboard + ManageCases: on network error, fall back to IndexedDB cached data + show offline banner
- App.jsx: online/offline indicator in top nav (green dot + "Online" / amber dot + "Offline")
- New files: `frontend/public/manifest.json`, `frontend/src/db.js`, `frontend/src/offlineSync.js`
...
- Phase 2 (offline CRUD + sync queue) is implemented:
  ...
  - Auto-sync triggers on `online` event; manual sync button always available
